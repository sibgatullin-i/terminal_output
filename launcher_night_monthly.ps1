...
$workFromHours = $settings.workFromHoursNight
$workFromMinutes = $settings.workFromMinutesNight
$workToHours = $settings.workToHoursNight
$workToMinutes = $settings.workToMinutesNight
...
while ($true){
    while ((get-date) -gt (get-date -hour $workFromHours -Minute $workFromMinutes -Second 0) -and (get-date) -lt (get-date -Hour $workToHours -Minute $workToMinutes -Second 0)){
        $lastStart = (get-date)
        & "$PSScriptRoot\upload_night_monthly.ps1"

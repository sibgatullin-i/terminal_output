...
Import-Module $PSScriptRoot\source.psm1 -DisableNameChecking
...
if (-not $InputFolder) {
  $InputFolder = FolderSelector -Path "$PSScriptRoot\input"
}
...
if (Test-Path $PSScriptRoot\tools\output.csv) {
...
while ($terminalCounter -ge 0) {
    & python.exe $PSScriptRoot\tools\output.py "$InputFolder"
